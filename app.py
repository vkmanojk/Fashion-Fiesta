...
def content_based_filtering(product_id):
    ...
    print('Product matched: ' + str(category_matched))
...
def tshirt():
    ...
    if 'view' in request.args:
    ...
    elif 'trial' in request.args:
        product_id = request.args['trial']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        ...
        temp2 = product
        temp3 = form
        return render_template('trial_product.html', x=x, tshirts=product, form=form)
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if picture.lower().endswith(('.png', '.jpg', 'jpeg')):
                save_photo = photos.save(file, folder='people')
                print('\nSaved image\n')
                cloth = 'static/image/product/'+temp2[0]['category']+'/'+temp2[0]['picture']
                print(picture)
                person = 'static/image/product/people/'+ picture
                ...
                url = url + '?person=' + person + '&clothing='+ cloth
                try:
                	print('Picture')
                	response = requests.post(url)
                	temp2[0]['picture'] = 'output.png'
                except:
            ...
            return render_template('trial_product.html')
    return render_template('trial_product.html', x=temp1, tshirts=temp2, form=temp3)
